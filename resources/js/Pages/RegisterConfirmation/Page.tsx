...
const StepperItem = ({
    ...
    const text = isActive || isCompleted ? "text-amber-700" : "text-amber-400";
    ...
    return (
        ...
        >
            <div
                className={`flex h-8 w-8 items-center justify-center rounded-full font-bold ${bubble}`}
            >
                {isCompleted ? "✓" : step}
            </div>
            <span className={`hidden sm:inline text-sm font-semibold ${text}`}>
                {label}
            </span>
        </div>
    );
...
const RegisterConfirmation: React.FC = () => {
    ...
    const submit = async () => {
        ...
        setSubmitting(false);
        router.visit("/register-success", { replace: true });
    };

    const back = () => router.visit("/register-upload");
                    ...
                    <div className="grid grid-cols-1 mt-5 gap-y-3 sm:gap-y-4 md:grid-cols-2 md:gap-x-8">
                        <div className="space-y-3 text-sm sm:text-base">
                            ...
                            <DataRow
                                ...
                                value={formatDate(f?.tanggalLahir)}
                            />
                            <DataRow
                                label="Nomor Handphone"
                                value={f?.noHandphone?.replace(
                                    /(\d{4})\d+/,
                                    "$1 XXXX XXXX"
                                )}
                            />
                            ...
                            <DataRow
                                ...
                                value={u?.formulirPendaftaran?.name}
                            />
                            <DataRow
                                label="Bukti Pembayaran"
                                value={u?.buktiPembayaran?.name}
                            />
                        </div>

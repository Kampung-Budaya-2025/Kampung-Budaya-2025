...
const StepperItem = ({
    ...
    return (
        ...
        >
            ...
            <span
                className={`hidden sm:inline text-sm font-semibold ${getTextColor()}`}
            >
                {label}
            </span>
        </div>
...
const FileUploadInput = ({
    ...
    <div>
        ...
        {data.error && (
            <p className="mt-1.5 text-xs text-red-600">{data.error}</p>
        )}
    </div>
);
...
const RegisterUpload: React.FC = () => {
    ...
    const validateFile = (
        ...
        if (type === "formulir") {
            ...
            if (file.size > maxSizeMB * 1024 * 1024)
                return {
                    isValid: false,
                    error: `Ukuran maksimal ${maxSizeMB}MB`,
                };
        } else {
            const allowedMimeTypes = [
                "image/jpeg",
    ...
    const handleNext = () => {
        ...
        router.visit("/register-confirmation");
    };

    const handleBack = () => {
        router.visit("/register-form");
    };
                ...
                <div className="relative w-full max-w-3xl p-6 mx-auto border shadow-lg bg-white/95 rounded-3xl border-amber-100 md:p-8">
                    ...
                    <div className="space-y-6">
                        <FileUploadInput
                            ...
                            onPick={(f) =>
                                handleFileUpload("formulirPendaftaran", f)
                            }
                        />
                        <FileUploadInput
                            ...
                            inputRef={buktiRef}
                            onPick={(f) =>
                                handleFileUpload("buktiPembayaran", f)
                            }
                        />
                    </div>
                </div>
